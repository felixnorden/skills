...
describe("tool_call wiring", () => {
	...
	test("ignores non-write/edit tools", async () => {
		const { dir, cleanup } = tempProject();
		const { fire } = stubPi();
		const event = { toolName: "bash", toolCallId: "t3", input: { command: "ls" } };
	...
	test("git-mutating bash commands invalidate the metadata cache", async () => {
		...
		const event = { toolName: "bash", toolCallId: "t4", input: { command: "git commit -m wip" } };
		expect(await fire(event, dir)).toBeUndefined();
	...
	test("read-only git bash commands keep the cache warm", async () => {
		...
		const event = { toolName: "bash", toolCallId: "t5", input: { command: "git status" } };
		await fire(event, dir);
		const readOnly = { toolName: "bash", toolCallId: "t6", input: { command: "git branch --show-current" } };
