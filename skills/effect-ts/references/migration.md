...
1. **Unified versioning** - All ecosystem packages share a single version number
2. **Package consolidation** - Platform, RPC, Cluster, and more merged into core `effect`
3. **ServiceMap** - New dependency injection system replacing Context
4. **Yieldable trait** - More explicit type safety for yieldable types
5. **Automatic fiber keep-alive** - No need for `runMain` in most cases
6. **Layer memoization** - Automatic across `Effect.provide` calls
7. **Unstable modules** - New features under `effect/unstable/*` paths
...
// With options
const fiber2 = yield* Effect.forkChild(task, {
